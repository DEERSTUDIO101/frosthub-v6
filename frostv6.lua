...
local InterfaceBuild = '1K1V'
local Release = "Build 1.01"
local FrostHubFolder = "FrostHub"
local ConfigurationFolder = FrostHubFolder.."/Configurations"
local ConfigurationExtension = "FrostHub.JSON"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(FrostHubFolder) then
				if isfile and isfile(FrostHubFolder..'/settings'..ConfigurationExtension) then
					file = readfile(FrostHubFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('FrostHub had an issue accessing configuration saving capability.')
		end
	end
end
...
local FrostHub = useStudio and script.Parent:FindFirstChild('FrostHub') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local FrostHubDestroyed = false -- True when FrostHubLibrary:Destroy() is called

repeat
	if FrostHub:FindFirstChild('Build') and FrostHub.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('FrostHub | Build Mismatch')
		print('FrostHub may encounter issues as you are running an incompatible interface version ('.. ((FrostHub:FindFirstChild('Build') and FrostHub.Build.Value) or 'No Build') ..').\n\nThis version of FrostHub is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, FrostHub = FrostHub, useStudio and script.Parent:FindFirstChild('FrostHub') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

FrostHub.Enabled = false

if gethui then
	FrostHub.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(FrostHub)
	FrostHub.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	FrostHub.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == FrostHub.Name and Interface ~= FrostHub then
			Interface.Enabled = false
			Interface.Name = "FrostHub-Old"
		end
	end
elseif not useStudio then
...
if FrostHub.AbsoluteSize.X < minSize.X and FrostHub.AbsoluteSize.Y < minSize.Y then
...
local Main = FrostHub.Main
local MPrompt = FrostHub:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = FrostHub:FindFirstChild('Drag')
...
local Notifications = FrostHub.Notifications

local SelectedTheme = FrostHubLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = FrostHubLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	FrostHub.Main.BackgroundColor3 = SelectedTheme.Background
	FrostHub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	FrostHub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	FrostHub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	FrostHub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	FrostHub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	FrostHub.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		FrostHub.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		FrostHub.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(FrostHub:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("FrostHub | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("FrostHub | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('FrostHub had an issue decoding the configuration file, please try delete the file and reopen FrostHub.') return end


    -- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(FrostHubLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("FrostHub | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--FrostHubLibrary:Notify({Title = "FrostHub Flags", Content = "FrostHub was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
	return changed
end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			FrostHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			FrostHubLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "FrostHubOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(FrostHubFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('FrostHub Settings', 0, true)

	if TabList['FrostHub Settings'] then
		TabList['FrostHub Settings'].LayoutOrder = 1000
	end

	if Elements['FrostHub Settings'] then
		Elements['FrostHub Settings'].LayoutOrder = 1000
	end
...
---------------------------------------------- *FUNKTIONEN* --------------------------
function FrostHubLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('FrostHub | Not seeing your actions in notifications?')
...
function FrostHubLibrary:CreateWindow(Settings)
	print('creating window')
	if FrostHub:FindFirstChild('Loading') then
		if getgenv and not getgenv().FrostHubCached then
			FrostHub.Enabled = true
			FrostHub.Loading.Visible = true

			task.wait(1.4)
			FrostHub.Loading.Visible = false
		end
	end

	if getgenv then getgenv().FrostHubCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				FrostHubLibrary:Notify({Title = 'Build Mismatch', Content = 'FrostHub may encounter issues as you are running an incompatible interface version ('.. ((FrostHub:FindFirstChild('Build') and FrostHub.Build.Value) or 'No Build') ..').\n\nThis version of FrostHub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "FrostHubOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "FrostHubOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(FrostHubFolder) then
		makefolder(FrostHubFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "FrostHub"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "FrostHub Interface Suite" then
		LoadingFrame.Version.Text = "FrostHub UI"
	end
	...
	if not Settings.DisableFrostHubPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				FrostHubLibrary:Notify({
					Title = "FrostHub Interface",
					Content = "Enjoying this UI library? Support us at https://discord.gg/Ab73VVR4JK",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(FrostHubFolder.."/Discord Invites") then
			makefolder(FrostHubFolder.."/Discord Invites")
		end

		if isfile and not isfile(FrostHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(FrostHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"FrostHub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(FrostHubFolder.."/Key System") then
			makefolder(FrostHubFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("FrostHub | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FrostHub specific development.')
				end
			end
		end
		...
		if isfile and isfile(FrostHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(FrostHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(FrostHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						FrostHubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called FrostHubLibrary:Destroy()
				if FrostHubDestroyed then
					return
				end
				if not Success then
					...
					print("FrostHub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FrostHub specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= FrostHubLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					FrostHubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			FrostHub.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("FrostHub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FrostHub specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					FrostHubLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("FrostHub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with FrostHub specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					FrostHubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("FrostHub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with FrostHub specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					FrostHubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= FrostHubLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("FrostHub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with FrostHub specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						FrostHubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("FrostHub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with FrostHub specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					FrostHubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			FrostHubLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			FrostHubLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('FrostHub had an issue creating settings.') end
...
function FrostHubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function FrostHubLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function FrostHubLibrary:Destroy()
	FrostHubDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['FrostHub Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "FrostHubOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function FrostHubLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				FrostHubLibrary:Notify({Title = "FrostHub Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			FrostHubLibrary:Notify({Title = "FrostHub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('FrostHub Configurations Error | '..tostring(result))
			FrostHubLibrary:Notify({Title = "FrostHub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
	...
	--local Window = FrostHubLibrary:CreateWindow({
	--	Name = "FrostHub Example Window",
	--	LoadingTitle = "FrostHub Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using FrostHub may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like FrostHub to get the key from
	...
	----FrostHubLibrary:Notify({Title = "FrostHub Interface", Content = "Welcome to FrostHub. These - are the brand new notification design for FrostHub, with custom sizing and FrostHub calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(FrostHubLibrary.Theme) do
...
task.delay(4, function()
	FrostHubLibrary.LoadConfiguration()
